...
X_train, X_test, y_train, y_test = train_test_split(
    csv_df["processed_text"], csv_df["sentiment"], test_size=0.1, random_state=42)
...
# token.fit_on_texts([str(text) for text in X_train])

vocabulary = len(token.index_word) + 1
print("Vocabulary size={}".format(len(token.word_index)))
print("Number of Documents={}".format(token.document_count))

# Sequence
X_train = token.texts_to_sequences([str(text) for text in X_train])
X_test = token.texts_to_sequences([str(text) for text in X_test])
...
model = Sequential()
model.add(Embedding(vocabulary, VOCABULARY_SIZE,
